...
def PSD_compare(real_data, fake_data):
    ...
    psd_real = PowerSpectralDensity(real_data)
    psd_fake = PowerSpectralDensity(fake_data)
    
    for c in range(channels):
        
        res[c] = np.sqrt((np.log10(psd_real[c]/psd_fake[c])**2).mean())
        
    return res

def PSD_compare_torch(real_data, fake_data):
    ...
    psd_real = PowerSpectralDensity(real_data.cpu().numpy())
    psd_fake = PowerSpectralDensity(fake_data.cpu().numpy())
