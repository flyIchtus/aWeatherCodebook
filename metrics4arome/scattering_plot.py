...
def plot_2o_Estimators(estim_fake, estim_real, name, J,L, output_dir):
    ...
    for i, var in enumerate(['u', 'v', 't2m']):

        print(var)
        fig, axs = plt.subplots(1, J-1, figsize=(21, 15),
        ...
        for j_up in range(1,J):
            ...
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_fake[:,i,si:ei].mean(axis=0), 'r-', label='Mean GAN', linewidth =2.5)
            
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_fake[:,i,si:ei].mean(axis=0)
                                    +estim_fake[:,i,si:ei].std(axis=0), 
                                    'r--', label=r'Mean $\pm$ Std GAN', linewidth =2.5)
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)],
               estim_fake[:,i,si:ei].mean(axis=0)
                                    -estim_fake[:,i,si:ei].std(axis=0), 'r--', linewidth =2.5)
            
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_fake[:,i,si:ei].max(axis=0), 'r+', label='Max/Min GAN', linewidth =2.5)
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_fake[:,i,si:ei].min(axis=0), 'r+', linewidth =2.5)
        

            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_real[:,i,si:ei].mean(axis=0), 'k-', label='Mean AROME-EPS', linewidth =2.5)
            
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_real[:,i,si:ei].mean(axis=0)
                 +estim_real[:,i,si:ei].std(axis=0), 
                 'k--', label='Mean $\pm$ Std AROME-EPS', linewidth =2.5)
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_real[:,i,si:ei].mean(axis=0)
                 -estim_real[:,i,si:ei].std(axis=0),'k--', linewidth =2.5)
            
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_real[:,i,si:ei].max(axis=0), 'k+',label='Max/Min AROME-EPS', linewidth =2.5)
            axs[J-1-j_up].plot([1+j2+(J-j_up) for j2 in range(j_up)], 
               estim_real[:,i,si:ei].min(axis=0), 'k+', linewidth =2.5)
            
            axs[J-1-j_up].set_xticks(list(range(J-j_up+1, J+1)), fontsize = '25')
            axs[J-1-j_up].set_xlabel(r"Second-order scale $j_2 > j_1$", fontsize = '25', fontweight = 'bold')
            axs[J-1-j_up].title.set_text(r"First-order scale $j_1$={}, variable {}".format(J-j_up, var))
            axs[J-1-j_up].title.set_size('32') #, fontweight = 'bold')
            axs[J-1-j_up].title.set_weight('bold')
        axs[0].set_ylabel("{} estimator".format(name), fontsize = '25', fontweight = 'bold')
        ...
        st=fig.suptitle('Scattering {} estimators for {}, $J$={}, $L$={}'.format(name,var, J,L), fontsize='28', fontweight = 'bold')
        st.set_y(1.0)
